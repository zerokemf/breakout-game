...
test('LIVE NAS: browser submits real score, reload and new browser retrieve same database row',async({browser})=>{
 ...
 await page.getByRole('button',{name:'開始遊戲',exact:true}).click();await page.evaluate(async()=>{const {game}=await import('./js/main.js?v=2a-zh');game.score.addScore(200);game.finish();});
